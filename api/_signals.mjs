...
function computeSuggestedPositionComposition({
  coin,
  accountSize,
  currentPrice,
  signals
  ...
  const provisionalSetup = confirmedSetup ? null : computeProvisionalSetup(coin, signals, currentPrice, { source: "live" });
  ...
  const modeExplanation = mode === "validated" ? "Suggested from the current confirmed setup and your account capital." : "Draft composition from the current directional bias. Size and leverage are reduced until setup confirmation improves.";
  if (!isFinite(accountSize) || accountSize <= 0) {
    return {
      hasSetup: true,
      setup: activeSetup,
...
function deriveCompositionRiskStatus(composition) {
...
function buildObservatorySnapshot(input) {
  ...
  const fundingAligned = alignSnapshots(times, input.fundingHistory, (snapshot) => snapshot.rate * 1e4);
...
function alignSnapshots(targetTimes, snapshots, readValue) {
  const output = Array(targetTimes.length).fill(null);
...
function buildEventMessage(label, kind, fromState, toState) {
