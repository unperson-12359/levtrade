...
import { SIGNAL_TEXT_CLASSES, SIGNAL_BG_CLASSES, SIGNAL_COLORS } from '../../utils/colors'
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {isWarmingUp && (
          <div className="rounded-lg border border-signal-yellow/20 bg-signal-yellow/5 p-4">
            <div className="flex items-center gap-3">
              <div className="text-signal-yellow text-base font-medium">Warming Up</div>
              ...
              <span className="text-sm text-text-muted font-mono">{(warmupProgress * 100).toFixed(0)}%</span>
            </div>
        ...
        {/* Staleness Warning */}
        {signals.isStale && (
          <div className="rounded-lg border border-signal-red/20 bg-signal-red/5 p-4 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="rounded-lg border border-border-subtle bg-bg-card p-6">
            ...
            <div className="mt-2 text-sm text-text-muted font-mono">
          ...
          <div className="rounded-lg border border-border-subtle bg-bg-card p-6">
            ...
            <div className="mt-4 flex items-center justify-center gap-6">
              ...
              <div className="text-center">
                ...
                <div className="font-mono text-sm font-bold mt-1" style={{ color: oiDelta.oiChangePct >= 0 ? 'var(--color-signal-green)' : 'var(--color-signal-red)' }}>
                  {formatPercent(oiDelta.oiChangePct * 100, 2)}
                </div>
                <div className="text-sm text-text-muted mt-0.5">Open Interest</div>
              </div>

              {/* Confirmation badge */}
              <div className={`px-3 py-1.5 rounded-lg text-base font-medium ${SIGNAL_TEXT_CLASSES[oiDelta.color]} ${SIGNAL_BG_CLASSES[oiDelta.color]}`}>
              ...
              <div className="text-center">
                ...
                <div className="font-mono text-sm font-bold mt-1" style={{ color: oiDelta.priceChangePct >= 0 ? 'var(--color-signal-green)' : 'var(--color-signal-red)' }}>
                  {formatPercent(oiDelta.priceChangePct * 100, 2)}
                </div>
                <div className="text-sm text-text-muted mt-0.5">Price</div>
              </div>
            </div>
        ...
        <div className="rounded-lg border border-border-subtle bg-bg-card p-6">
              ...
              <div className="flex items-center justify-center gap-4 mt-3">
                {composite.signalBreakdown.map((sig) => (
                  <div key={sig.name} className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm text-text-secondary">{sig.name}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Decision sentence */}
          <div className="mt-5 rounded-lg border-l-4 p-6" style={{ borderLeftColor: SIGNAL_COLORS[composite.color], backgroundColor: 'rgba(0,0,0,0.2)' }}>
