...
export function getSignalProvenance(interval: CandleInterval): Record<SignalSeriesKind, SignalProvenance> {
  const tf = interval
  return {
    zScore: {
      title: 'Price Z-Score (Price Stretch)',
      description:
        `How many standard deviations the current price is from its 20-period average. Values above +2 or below -2 suggest the price has stretched far and may snap back.`,
      source: `Computed from Hyperliquid ${tf} candles.`,
      unit: 'σ',
      requestType: 'candleSnapshot',
      timeframe: tf,
      lookback: '20 periods',
      updateCadence: `Live mids update continuously; candles refresh every ${tf} with 60s polling.`,
      referenceLines: [
    ...
    atr: {
      ...
      description:
        `Rolling 14-period Average True Range. This measures typical ${tf} price movement and drives stop placement and volatility context.`,
      source: `Computed from Hyperliquid ${tf} OHLC candles.`,
      unit: '$',
      requestType: 'candleSnapshot',
      timeframe: tf,
      lookback: '14 periods',
    ...
    stretchZ: {
      ...
      source: `Computed from Hyperliquid ${tf} candles and ATR.`,
      unit: 'σ',
      requestType: 'candleSnapshot',
      timeframe: tf,
      lookback: '20 periods',
