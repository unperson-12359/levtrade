...
function crossoverSeries(fast: Series, slow: Series): Series {
...
function squeezeSeries(bbWidth: Series, threshold: number): Series {
  const result: Series = [null]
...
function bodyRatioZSeries(candles: Candle[], period: number): Series {
  ...
  for (let i = 0; i < bodies.length; i++) {
    ...
    const mean = sum / period
    let sumSq = 0
    for (let j = i - period; j < i; j++) { const d = bodies[j]! - mean; sumSq += d * d }
    const std = Math.sqrt(sumSq / period)
    if (std < 1e-9) { result.push(0); continue }
    result.push((bodies[i]! - mean) / std)
  }
  return result
}

function donchianBreakSeries(donchianPos: Series): Series {
